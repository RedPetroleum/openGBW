...
#define GAME_MAX_FRAME_DT 0.1f         // longer frames slow the game down instead of letting comets jump
...
#define GAME_OVER_INPUT_DELAY_MS 800   // clicks are ignored this long after dying (no accidental retry)

// Playfield
#define GAME_FIELD_TOP 10              // rows above are used by the energy bar, lives and score
...
static bool bitmapPixelSet(const uint16_t *rows, int width, int row, int col)
{
...
static void drawPixelClipped(int x, int y, int top = GAME_FIELD_TOP)
{
  if (x >= 0 && x < 128 && y >= top && y <= GAME_FIELD_BOTTOM)
    screen.drawPixel(x, y);
}

static void drawBitmap(const uint16_t *rows, int width, int height, int x, int y, int top = GAME_FIELD_TOP)
{
...
static void resetGame()
{
  if (!bestScoreLoaded)
  {
    ...
    bestScore = preferences.getUInt("gameScore", 0);
    preferences.end();
...
// Returns how many grams the scale is pressed; while not pressed the zero point follows scale drift
static float pressedWeight(float dt)
{
...
static void destroyShip()
{
  ...
  if (score > bestScore)
  {
    ...
    newBestScore = true;
    preferences.begin("scale", false);
    preferences.putUInt("gameScore", bestScore);
    preferences.end();
  }
...
static void drawReady(unsigned long now)
{
  char buf[4];
  screen.setFontPosTop();
  screen.setFont(u8g2_font_7x14B_tf);
  CenterPrintToScreen("COMET BLASTER", 12);
  screen.setFont(u8g2_font_5x7_tr);
  CenterPrintToScreen("Knob: move ship", 30);
  CenterPrintToScreen("Press scale: laser", 39);
  screen.setFont(u8g2_font_7x14B_tf);
  snprintf(buf, sizeof(buf), "%lu", 3 - (now - stateStartedAt) * 3 / GAME_READY_MS);
  CenterPrintToScreen(buf, 49);
}

// Screen with a title, one line of text and two options selected with the knob
static void drawChoiceScreen(const char *title, const char *text, const char *firstOption, const char *secondOption)
{
  screen.setFontPosTop();
  screen.setFont(u8g2_font_7x14B_tf);
...
static void selectChoice(int steps)
{
  if (steps > 0)
    menuChoice = 1;
  else if (steps < 0)
...
void gameLoop()
{
  ...
  if (steps != 0 || click || gameState == GAME_READY || gameState == GAME_RUNNING || gameState == GAME_EXPLODING)
    lastSignificantWeightChangeAt = now;
  ...
  case GAME_READY:
    moveShip(steps, dt);
    pressedWeight(dt); // settle the zero point before the laser is used
  ...
  case GAME_PAUSED:
    ...
    drawChoiceScreen("Paused", buf, "Continue", "Exit");
    break;
  ...
  case GAME_OVER:
    ...
    drawChoiceScreen("Game Over", buf, "Retry", "Exit");
    break;
  }
}
